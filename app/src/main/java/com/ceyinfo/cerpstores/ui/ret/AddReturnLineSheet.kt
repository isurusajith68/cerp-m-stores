...
import com.ceyinfo.cerpstores.data.model.CreateReturnLineRequest
import com.ceyinfo.cerpstores.data.model.Material
import com.ceyinfo.cerpstores.databinding.SheetAddReturnLineBinding
import com.ceyinfo.cerpstores.ui.common.MaterialPickerSheet
import com.google.android.material.bottomsheet.BottomSheetDialog

object AddReturnLineSheet {

    data class LineEntry(
        val material: Material,
        val request: CreateReturnLineRequest,
    )

    fun show(activity: Activity, onSave: (LineEntry) -> Unit) {
        val dialog = BottomSheetDialog(activity, R.style.Theme_Stores_BottomSheet)
        val binding = SheetAddReturnLineBinding.inflate(LayoutInflater.from(activity))
        ...
        binding.btnSave.setOnClickListener {
            ...
            if (material == null) {
                Toast.makeText(activity, R.string.return_line_validation_material, Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            if (qty <= 0.0) {
                binding.tilQty.error = activity.getString(R.string.return_line_validation_qty)
                return@setOnClickListener
            }
            binding.tilQty.error = null

            val req = CreateReturnLineRequest(
